        ...
        fetch(`/api/admin/products/${id}`)
            ...
            .then(product => {
                ...
                if (product.highlights && product.highlights.length > 0) {
                    setHighlights(product.highlights.map(h => ({ icon: h.icon || '✨', text: h.text || '' })))
                }

                if (product.specs && product.specs.length > 0) {
                    setSpecs(product.specs.map(s => ({ key: s.key || '', value: s.value || '' })))
                }
    ...
    if (fetching) {
        return (
            <div className="flex items-center justify-center py-20">
                <div className="text-text-muted text-lg">Loading product data...</div>
            </div>
        )
    }
    ...
    return (
        <div className="pb-20">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-3xl font-black font-[Outfit]">Edit Product</h1>
                    <p className="text-text-secondary">Update the product listing details.</p>
                </div>
            </div>
