...
function ProductCard({ product, delay }) {
    ...
    return (
        ...
        hover:-translate-y-1.5 hover:shadow-[0_12px_40px_rgba(0,0,0,0.5)] hover:border-accent-yellow/15">
                {/* Image */}
                <Link to={`/product/${product.slug}`} className="block relative w-full h-60 overflow-hidden bg-[#16161a]">
            ...
            hover:bg-accent-yellow hover:text-[#111] hover:border-accent-yellow transition-all duration-350 whitespace-nowrap cursor-pointer">
                ...
                {/* Body */}
                <div className="p-5">
                    <Link to={`/product/${product.slug}`}><h4 className="text-base font-semibold mb-2 hover:text-accent-yellow transition-colors">{product.name}</h4></Link>
