    ...
    return (
        <>
                ...
                <section className="max-w-[1280px] mx-auto px-6 pb-20">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                      ...
                      ${rfqSent
                                    ...
                                    <button onClick={() => setShowRfqModal(true)} className="w-full py-3.5 rounded-full font-semibold text-sm border-2 border-white/20 text-text-primary hover:border-accent-yellow hover:text-accent-yellow transition-all duration-300 cursor-pointer">
                                        📦 Request Bulk Quote
                                    </button>
                                </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            {relatedProducts.map((p, i) => (
                                <ScrollReveal key={p.name} delay={i * 80}>
                                    <Link href={`/product/${p.slug}`} className="block group bg-bg-card border border-border-glass rounded-[20px] overflow-hidden hover:-translate-y-1.5 hover:shadow-[0_12px_40px_rgba(0,0,0,0.5)] hover:border-accent-yellow/15 transition-all duration-400">
                                        ...
                                        <div className="p-5">
                                            ...
                                            <span className="block w-full py-2 rounded-full text-xs font-semibold text-center bg-accent-yellow/10 text-accent-yellow border border-accent-yellow/30 group-hover:bg-accent-yellow group-hover:text-[#111] transition-all duration-300 cursor-pointer">
                                                📋 Request Quotation
                                            </span>
                                        </div>
                                    </Link>
          ...
          ═══════════════════════════════════════════ */}
            ...
            {
                showRfqModal && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" onClick={() => setShowRfqModal(false)}>
                        <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
                        <div className="relative bg-[#16161a] border border-border-glass rounded-[24px] w-full max-w-md overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.6)]" onClick={e => e.stopPropagation()}>
                            {/* Modal Header */}
                            <div className="bg-gradient-to-r from-accent-yellow/10 to-accent-orange/10 border-b border-border-glass p-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h3 className="text-xl font-black font-[Outfit]">Request Quotation</h3>
                                        <p className="text-text-secondary text-sm mt-1">We'll get back to you within 24 hours</p>
                                    </div>
                                    <button onClick={() => setShowRfqModal(false)} className="w-8 h-8 rounded-full bg-white/[0.06] flex items-center justify-center text-text-muted hover:text-text-primary hover:bg-white/[0.12] transition-colors cursor-pointer">
                                        ✕
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="px-6 pt-5 pb-3">
                                <div className="flex items-center gap-3 bg-white/[0.04] border border-border-glass rounded-xl p-3">
                                    <img src={product.images?.[0]} alt={product.name} className="w-14 h-14 rounded-lg object-cover" />
                                    <div className="flex-1 min-w-0">
                                        <p className="font-semibold text-sm truncate">{product.name}</p>
                                        <p className="text-text-muted text-xs mt-0.5">Size: {selectedSize || 'N/A'} · Qty: {qty}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleRFQ} className="p-6 pt-3 space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setRfqForm({ ...rfqForm, customerName: e.target.value })}
                                        className="w-full px-4 py-3 bg-[#111] border border-border-glass rounded-xl text-text-primary text-sm outline-none focus:border-accent-yellow transition-colors"
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-text-secondary mb-1.5">Message (Optional)</label>
                                    <textarea
                                        rows={2}
                                        value={rfqForm.message}
                                        onChange={e => setRfqForm({ ...rfqForm, message: e.target.value })}
                                        className="w-full px-4 py-3 bg-[#111] border border-border-glass rounded-xl text-text-primary text-sm outline-none focus:border-accent-yellow transition-colors resize-none"
                                ...
                                <button
                                    type="submit"
                                    disabled={rfqLoading}
                                    className="w-full py-3.5 rounded-full font-bold text-sm bg-gradient-to-br from-accent-yellow to-accent-orange text-[#111] hover:-translate-y-0.5 hover:shadow-[0_6px_28px_rgba(245,197,24,0.35)] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 cursor-pointer"
                                >
                                    {rfqLoading ? 'Sending...' : '📋 Submit Quotation Request'}
                                </button>
                                <p className="text-center text-text-muted text-xs">Your info is safe & never shared with third parties.</p>
                            </form>
                        </div>
                    </div>
                )
